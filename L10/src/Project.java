...
public class Project {
    ...
    public void completeProject(){
    	...
    	while (this.linesOfCode > linesWritten || this.linesOfCode > linesTested) {
    		this.daysWorkedSoFar++;
    		for(int i = 0; i < this.employeesOnProject.size(); i++) {
    			if (this.employeesOnProject.get(i) instanceof Programmer) {
    				linesWritten += this.employeesOnProject.get(i).work();
    			}
    			if(this.employeesOnProject.get(i) instanceof Tester) {
    				linesTested += this.employeesOnProject.get(i).work();
    			}
    		}
    	}
    }
    ...
    public String toString(){
        ...
        s += "Deadline: " + this.deadline + "\n";
        s += "Days required: " + this.daysWorkedSoFar + "\n";
        s += "Employees on Project: " + "\n";
        for (int i = 0; i < this.employeesOnProject.size(); i++) {
        	s += this.employeesOnProject.get(i).toString() + "\n";
        }
    	return s;
    }
    ...
    public static void main(String [] args) throws FileNotFoundException {
        ...
        Project p2 = new Project(6000, 16, 11, 11);
        Project p3 = new Project(4000, 14, 9, 9);
        p2.completeProject();
        p3.completeProject();
