...
public class Recursion {
    ...
    public static double geometricSum(int n){
    	...
    	return (1/(Math.pow(2, m))) + geometricSum(n - 1);
    }
    ...
    public static boolean isPalindrome(String word){
         ...
         if (word.length() == 2) {
        	 String a = word.subSequence(0, 1).toString(), b = word.substring(1, 2).toString();
         ...
         String a = word.subSequence(0, 1).toString(), b = word.substring(word.length() - 1, word.length()).toString();
    	if (!a.equalsIgnoreCase(b)) {
    		return false;
    	}
    	
         
    	return isPalindrome(word.subSequence(1, word.length() - 1).toString());
    }
    ...
    public static int longestWordLength(String words){
    	...
    	return lwlHelper1(wordSplit, 0, 0);
    }
    
   
    
    public static int lwlHelper1(String[] wordSplit, int n, int max) {
		if (n == wordSplit.length ) {
			return max;
		}
		
		if (wordSplit[n].length() > max) {
			max = wordSplit[n].length();
		}
		
		return lwlHelper1(wordSplit, n + 1, max);
    }
